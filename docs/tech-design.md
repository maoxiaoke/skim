...
> 阶段：Scrum Phase 5 ｜ 日期：2026-06-12 ｜ 状态：待评审
...
| 用例 | 触发者 | 读/写 | 落点 |
|---|---|---|---|
| 扫描/盘点、查看详情 | 启动 / 定时 / 手动刷新 | 读 | skills 目录 + agent 配置 + 会话记录 |
...
interface SkillRecord {
  ...
  sizeBytes: number;
  fileCount: number;
  flags: { duplicate: boolean; strayFile: boolean; parseError: boolean; locked: boolean };
}
...
| `~/.claude/settings.json` + 项目 `settings.json` / `settings.local.json` | JSON 解析，`skillOverrides` 三层合并（local > project > user） |
| `~/.codex/config.toml` | TOML 解析 `[[skills.config]]`，path 归一化（接受目录或 SKILL.md 两种写法，本机实测两种并存） |
